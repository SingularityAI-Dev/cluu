# Stack Research — Cluu v1

**Domain:** Browser 2D cozy game + LLM grading + Supabase auth/state
**Researched:** 2026-04-20
**Confidence:** HIGH for versioned libraries; MEDIUM for stack-level prescriptions (one deviation from locked design doc required — see "Critical deviations from design doc §8" below)

---

## Critical deviations from design doc §8

The design doc (locked 2026-04-20) prescribes **Next.js 14**. As of April 2026, this is no longer viable. Two deviations are required; the rest of §8 stands.
...
Everything else in §8 verifies cleanly.
...
# State + utilities
pnpm add zustand@^5.0.12 zod@^3.23.0 gray-matter@^4.0.3

# Rate limit + cache
pnpm add @upstash/redis@^1.34.0 @upstash/ratelimit@^2.0.0
...
pnpm add @logic-md/core@workspace:*

# Dev
pnpm add -D vitest@^2 @vitest/ui
```

`next/og` is built into Next 16 — no install needed for OG images.
...
- Recommended pattern (from official `phaserjs/template-nextjs`):
  - `app/play/page.tsx` (Server Component) imports `<PhaserGame />` via `next/dynamic({ ssr: false })`.
  - `PhaserGame` ref exposes `{ game, scene }` via `forwardRef` + `useImperativeHandle` so React UI panels (HUD, encounter modal) can dispatch to Phaser scenes.
  - React ↔ Phaser communication uses an **EventBus** (`Phaser.Events.EventEmitter` singleton), NOT prop drilling.
...
- In your `app/api/encounter/grade/route.ts`:
  ...
  - `export const maxDuration = 60` is plenty for Sonnet gen + Haiku grade. A full round trip is usually <10s.
  - Use `client.messages.stream({...}).toReadableStream()` and return a `Response` with it — first byte must flush within 25s to preserve streaming on Fluid.
...
- Model IDs (verified 2026-04-20):
  - Generation: `claude-sonnet-4-6` (released 2026-02-17; 1M context; 64k max output).
  - Grading: `claude-haiku-4-5` (or dated `claude-haiku-4-5-20251001`). Prefer the alias in code; pin dated ID in tests for regression safety.
...
- Pattern:
  ```ts
  const { success, reset } = await ratelimit.limit(`grade:${userId}`);
  if (!success) return Response.json({ error: 'rate_limited', resetAt: reset }, { status: 429 });
  ```
...
**If you want a single front-door for all Anthropic calls (observability, model fallbacks, BYOK in v1.1):**
...
**If Anthropic SDK streaming is flaky on Fluid:**
...
**If encounter count grows past ~100 in v1.3:**
...
**If iPhone SE 2020 60fps target slips:**
...
**MEDIUM confidence (pattern-level, multiple community sources agree):**
...
**LOW confidence (flagged for phase-specific validation later):**
...
*Stack research for: Cluu v1 — cozy browser 2D game + LLM-graded prompt encounters*
*Researched: 2026-04-20*
*Prescriptive intent: solo builder should be able to run the install block above and have a correct, current 2026 stack without further decisions.*
